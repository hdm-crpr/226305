...
# PRO-TIPP: Sie können das Skript einfach übernehmen, indem sie Ihr igraph-Objekt ebenfalls "h" als variable zuweisen. Dann kann das Skript direkt eingelesen werden.
...
# LEKTRION 1. Vergabe von festen Visualisierungsparametern für edge- und vertex-Attribute. diese werden einmal angelegt und dann von jedem plot übernommen. Beispielsweise Farbe oder Form von Knoten und Kanten, z.B. alle weiblichen Knoten sollen ein Kreis sein, alle männlichen Knoten ein Dreieck (vertex.shape).
...
E(h)$width <- E(h)$weight # definiert die Kantenstärke nach dem hinterlegten Gewicht.
...
# Tipp: Überlegen Sie zunächst, welche Variable für die größere Anzahl im Netzwerk gilt. Selektrieren Sie dann die zweite Form danach.
...
inh
plot(h, vertex.size=inh*1.5, vertex.label=NA)
...
# Pfaddistanzen bereichnen
d18 <- distances(h, v=V(h)[name=="18"], to=V(h), weights=NA)
# Farben definieren
fine = 5
palette = colorRampPalette(c('lightgreen','darkgreen'))
dcol = palette(fine)[as.numeric(cut(d18, breaks = fine))] 
plot(h, vertex.color=dcol, vertex.label=d18, vertex.label.color="white", vertex.frame.color = "white", edge.color="grey80", main = "Pfaddistanz von Knoten 18")
