...
weight # zeigt alle Gewichte der Edges im Netzwerk an
...
# Benennen Sie die Edge- und Vertex-Attribute aus ihrem Netzwerk in einfache Namen um, in dem Sie die Zuweisungsfunktion von oben verwenden.
...
# Pro-Tipp: Sie sehen die Werte auch in der Übersicht im igraph-Objekt abgetragen vor der Kurzbeschreibung des Netzwerks (34 78)
...
# dieser Wert sagt uns, dass die Dichte des Netzwerks 13,90% beträgt, d.h. es wurden von allen möglichen Beziehungen zwischen den Knoten nur knapp jede zehnte realisiert.

## POSITIONALE bzw. AKTEURSbezogene Maße (bezogen auf die Position einzelner Knoten im Netzwerk)

# Wer ist am stärksten im Netzwerk vernetzt? (allgemeiner Degree-Wert)
degree(karate)
# hier werden einfache Häufigkeiten im Netzwerk gezählt.
...
centr_clo(karate, mode = "all")$centralization
...
## ÜBUNG: Führen Sie die Analyse Ihres Netzwerks nach den Maßen oben durch.
...
ego(karate, order = 2, nodes = V(karate)$name == "Mr Hi", mode = "all")
...
mrhi
plot(mrhi[[1]]) # der Visualisierungsbefehl ist hier etwas umständlich.
...
# ÜBUNG: Führen Sie einen Triadenzensus im Netzwerk durch. Bilden Sie ein bis zwei Ego-Netzwerke
...
# ÜBUNG: Selektrieren Sie sinnvolle Teilnetzwerke aus Ihrem Netzwerk
