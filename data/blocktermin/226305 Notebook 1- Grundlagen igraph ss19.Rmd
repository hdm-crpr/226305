...
Dies ist ein R Markdown Dokument. Markdown ist eine interaktive Sprache, mit der man R-Code direkt ausführen kann. Das elegante: alles wird in plain text, d.h. ohne Formatierung im Dokument geschrieben. Die Formatierung bzw. das Rendering des Textes und des Codes übernimmt das Programm. RMarkdown eignet sich deshalb auch sehr gut für die Erstellung von Forschungsberichten oder ähnlichem in einer Gruppe (gemeinsam mit einer Versionsverwaltung).
...
Ergänzen von Code (z.B. für den finalen Forschungsbericht)
Cmd/Ctrl + Alt + i
...
# Selbsttest: Manipulieren Sie direkt im Skript oben die Werte für n=18 und power=1. Was passiert, wenn 35 Personen im Netzwerk sind und die Power den Wert 2 hat?
```
...
# Wir haben hier eine einfache Visualisierung mit einem kleinen Befehl eingeführt
# ifelse definiert, dass die Ausprägung entsprechend dargestellt werden soll. 
...
# gute Erklärung zu two-mode/bipartite Netzwerken hier:
...
R hat bereits eingebaute Datensätze unter dem Paket igraphdata. Das Paket muss allerdings zuvor installiert werden.  Wichtig ist, dass die Codierung des Datensatzes klar nachvollziehbar ist. Es gibt viele Fundstellen für existierende Datensätze im Netz, die kostenlos weiterverwendet werden können https://www.kaggle.com/datasets.

### Kapitel 2.2.2 Einlesen von CSV-Dateien

Jetzt benötigen wir einen eigenen Datensatz, den wir bearbeiten wollen. Dieser Datensatz wurde zuvor auf github erstellt. Er analysiert 
...
Für dieses Beispiel liegen die CSV-Dateien auf einem öffentlihc zugänglichen Server. Alternativ lassen sich die Datei auch in einem lokalen Verzeichnis bzw. der working directory ablegen. 
...
# Einlesen des Datensatzes von github: unbedingt sicherstellen, dass a) der Pfad korrekt ist und b) der Datensatz ausschließlich von raw.github... eingelesen wird, damit R einen direkten Zugriff auf die Datei hat
