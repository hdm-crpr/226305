...
plot(s, edge.arrow.size=.1, layout=layout_with_kk, main=" Studierende CR/PR, 3. Semester / Vollerhebung", sub="n=36 Vertices, KK-Algorithmus, ohne Gewicht, beide Fragen (Ratsuche und Zusammenarbeit), 144 Edges")
...
plot(s,
     ...
     sub="n=36, rot = Ratsuchenetzwerk, blau = Zusammenarbeit")
...
plot(s, edge.arrow.size=.2, layout=layout_with_kk, main="Beispiel Studierende", sub="KK-Algorithmus, Kantenstärke gewichtet")
...
plot(help,
     ...
     sub="n=36, KK-Algorithmus, Kantenstärke gewichtet")
...
# legt fest, dass die Visualisierung in einer Tabelle von 2x1 Feldern definiert werden
...
plot(help_w3, layout=layout_with_kk, vertex.color="red", main="Ratsuche-Netzwerk: Erste Nennung", sub="n=36, KK-Algorithmus")

plot(work_w3, layout=layout_with_kk, main="Zusammenarbeits-Netzwerk: Erste Nennung", sub="n=36, KK-Algorithmus")

plot(help_w1, layout=layout_with_kk, vertex.color="red", main="Ratsuche-Netzwerk: Zweite Nennung", sub="n=36, KK-Algorithmus")

plot(work_w1, layout=layout_with_kk,  main="Zusammenarbeits-Netzwerk: Zweite Nennung", sub="n=36, KK-Algorithmus")
...
# A Maße über das gesamte Netzwerk:

# Wie viele Komponenten hat das Netzwerk?

components(s)
# Netzwerk (s) hat nur eine Komponenten
is.connected(s)
...
edge_density(s)
# Die Dichte in unserem Netzwerk beträgt 11.43 % (gerundet), d.h. von allen möglichen Beziehungen zwischen den Knoten sind 11.43 realisiert.
...
# Wir vergleichen nun die unterschiedlichen Zentralitätsmaße. Die Knotengröße entspricht dabei dem Wert des entsprechenden Zentralitätsmaß. Die Werte dafür werden wir zentralisieren, so dass sie sich zwischen 0 und 1 bewegen
...
modularity(gh)
membership(gh)
plot(gh, help, edge.arrow.size=.2, main="Clusteranalyse Ratsuche")

gw <- cluster_walktrap(work)
modularity(gw)
membership(gw)
par(mfrow=c(1,1), mar=c(0,0,0,0))
plot(gw, work, edge.arrow.size=.2, main="Clusteranalyse Ratsuche")
