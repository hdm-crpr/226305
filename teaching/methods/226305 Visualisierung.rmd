...
####################################
# Visualisierung und Teilnetzwerke #
####################################
...
# Das Edge-Attribut "relation" hat jetzt noch die Werte der Kategorie 1=work, 2=help und 3=love. Da wir nicht die Gewichte von "love" mitaufkalkulieren wollen, löschen wir die Knoten, die die Kategorie 3 als Edge-Attribute in relation genannt haben. Dazu benötigen wir den Befehl subgraph.edges() und den logischen Operator != ungleich

s_hw_clean <- subgraph.edges(s, E(s)[relation!=3]) # löscht alle Werte, die nicht 3 sind (logischer Operator ungleich). 
s_hw_clean
...
V(s)[V(s)$sex == 1]$shape <- "vrectangle" # weiblich
V(s)[V(s)$sex == 2]$shape <- "crectangle" # männlich
V(s)[V(s)$sex == 2]$shape <- "circle" # männlich
...
### Vergleich zweier Netzwerke nach Edge-Attributen
...
V(s)$color <- col_sex[V(s)$sex] # selektiert die Vertex-Farbe nach den Farben im Vector colrs
